...
/*   Updated: 2024/05/16 14:32:42 by yfang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_philo(t_master *master, t_philo *philo)
{
	...
	while (++i < master->nbr_philo)
	{
		...
		philo[i].mutex_last_eat = malloc(sizeof(pthread_mutex_t));
		if (!philo[i].mutex_last_eat)
			ft_error(2);
...
void	ft_init_mutex(t_master *master)
{
	...
	if (!master->forks || !master->mutex_dead || !master->mutex_finish)
		ft_error(2);
